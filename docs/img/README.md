# Screenshots

Place screenshots used by [docs/screenshots.md](../screenshots.md) in this folder.

Please ensure screenshots contain no restricted/sensitive data.

## Usage policy (public record)
- Only include assets approved for public release.
- Do not include patient/participant data, PHI/PII, internal hostnames, application/registry URLs, credentials, or proprietary datasets.
- Prefer neutral, non-identifying example workspaces and UI states.
- Prefer PNG for screenshots; prefer SVG for logos/diagrams where possible.
- Recommended minimum width for screenshots: 1200px.

## Branding assets
The following logo assets are provided for the public record repository:
- `SciNetX_logo_icon_only_square.svg`
- `SciNetX_logo_icon_only_square_1024.png`
- `SciNetX_logo_wordmark_below_square.svg`
- `SciNetX_logo_wordmark_below_square_1024.png`

Suggested social preview image (GitHub Settings → Social preview): `SciNetX_logo_wordmark_below_square_1024.png`.
